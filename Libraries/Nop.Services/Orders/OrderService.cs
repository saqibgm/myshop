...
using static Nop.Core.Domain.Catalog.CustomerProducts;
...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderService : IOrderService
    {
        ...
        public virtual IList<Order> GetAllOrders()
        {
            return _orderRepository.Table.ToList();
        }
        public virtual IList<Order> GetWeclappNewOrders(int noOfOrders, DateTime timeAfter, bool retryFailed = false)
        {
            var orders = _orderRepository.Table.OrderByDescending(i => i.Id).Where(o => string.IsNullOrEmpty(o.Wid) || (retryFailed && o.Wid == "-1") || (o.WSynced==false));
            //var orders = _orderRepository.Table.OrderByDescending(i => i.Id).Where(o => string.IsNullOrEmpty(o.Wid) || (retryFailed && o.Wid == "-1") || (o.OrderNotes.Any(n => n.CreatedOnUtc >= timeAfter)));
            if (noOfOrders > 0)
        ...
        {
            return SearchOrders2(storeId, vendorId, customerId, 0, productId, affiliateId, warehouseId,
             billingCountryId, paymentMethodSystemName, createdFromUtc, createdToUtc, osIds, psIds, ssIds,
             billingPhone, billingEmail, billingLastName, orderNotes, pageIndex, pageSize, getOnlyTotalCount);
        }

        public virtual IPagedList<Order> SearchOrders2(int storeId = 0,
            int vendorId = 0, int customerId = 0, int salemanId = 0,
            int productId = 0, int affiliateId = 0, int warehouseId = 0,
            int billingCountryId = 0, string paymentMethodSystemName = null,
            DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
            List<int> osIds = null, List<int> psIds = null, List<int> ssIds = null,
        ...
        public virtual List<CustomerProductDetailsModel> GetCustomerProducts(int customerId)
        {
            var query = from oi in _orderItemRepository.Table
                        ...
                        select new { p.Id, ProductName = p.Name, p.Sku, p.ManufacturerPartNumber, VendorName = v.Name, o.CreatedOnUtc, oi.UnitPriceExclTax, oi.Quantity } into i
                        group i by new { i.Id } into g
                        select new CustomerProductDetailsModel
                        {
                            Id = g.Key.Id,
                            ProductName = g.FirstOrDefault().ProductName,
                            Sku = g.FirstOrDefault().Sku,
                            ManufacturerPartNumber = g.FirstOrDefault().ManufacturerPartNumber,
                            VendorName = g.FirstOrDefault().VendorName,
                            LastOrderOn = g.FirstOrDefault().CreatedOnUtc,
                            LastPrice = g.FirstOrDefault().UnitPriceExclTax,
                            OrderCount = g.Count(),
                            Quantity = g.Sum(a => a.Quantity)
                        };
            List<CustomerProductDetailsModel> list = query.OrderByDescending(o => o.LastOrderOn).ToList();
            //database layer paging
            return list;
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="order">The order</param>
        public virtual void UpdateOrder(Order order, bool reCalProfit)
        {
            if (reCalProfit)
            {
        ...
        public virtual void UpdateOrder(Order order)
        {
            ...
            order.WSynced = order.WSynced == null ? true : false;
        ...
        public virtual void UpdateOrders(IList<Order> orders)
        {
            if (orders == null)
        ...
        public virtual List<OrderItem> GetLastPricesByCustomer(int customerId, int[] productIDs = null)
        {

            var query = _orderItemRepository.Table;

            query = _orderItemRepository.Table.Where(o => o.Order.CustomerId == customerId && o.Order.OrderStatusId == (int)OrderStatus.Complete && (productIDs == null || productIDs.Contains(o.ProductId)))
                    .GroupBy(x => x.ProductId)
                    .Select(x => x.OrderByDescending(y => y.Order.CreatedOnUtc)
                    .First());
