...
namespace Nop.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
        ...
        /// <summary>
        /// Prepare details to place Quick Order order. It also sets some properties to "processPaymentRequest"
        /// </summary>
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Details</returns>
        protected virtual PlaceOrderContainer PreparePlaceQuickOrderDetails(ProcessPaymentRequest processPaymentRequest, List<OrderItem> orderItems = null)
        {
            var details = new PlaceOrderContainer
            {
                //customer
                Customer = _customerService.GetCustomerById(processPaymentRequest.CustomerId),
                SalemanId = processPaymentRequest.SalesmanId
            };
            if (details.Customer == null)
                throw new ArgumentException("Customer is not set");
            ...
            if (affiliate != null && affiliate.Active && !affiliate.Deleted)
                details.AffiliateId = affiliate.Id;
            // Get PaymentMethod of Customer if not set from Quick Order
            if (string.IsNullOrEmpty(processPaymentRequest.PaymentMethodSystemName))
            {
                processPaymentRequest.PaymentMethodSystemName = _genericAttributeService.GetAttribute<string>(details.Customer,
                      NopCustomerDefaults.SelectedPaymentMethodAttribute, processPaymentRequest.StoreId);
            }
            //check whether customer is guest
            if (details.Customer.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed)
                throw new NopException("Anonymous checkout is not allowed");
            ...
            if (details.CustomerLanguage == null || !details.CustomerLanguage.Published)
                details.CustomerLanguage = _workContext.WorkingLanguage;
            // If no Billing Address then addressneed to make billing address
            if (details.Customer.BillingAddress == null)
            {
                // Modified by SI
                Address adr = details.Customer.Addresses.Where(a => !string.IsNullOrEmpty(a.CustomAttributes)).FirstOrDefault();
                if (adr == null)
                {
                    adr = details.Customer.Addresses.FirstOrDefault();
                }
                if (adr != null)
                {
                    details.Customer.BillingAddress = adr;
                    details.Customer.BillingAddress.Email = details.Customer.Email;
                    _customerService.UpdateCustomer(details.Customer);
                }// Customer Address available then updated
            }

            //billing address
            if (details.Customer.BillingAddress == null)
                throw new NopException("Billing address Or Address is not provided");

            if (!CommonHelper.IsValidEmail(details.Customer.BillingAddress.Email))
                throw new NopException("Email is not valid");
            // Modified by SI
            Address custAdr = details.Customer.Addresses.Where(a => !string.IsNullOrEmpty(a.CustomAttributes)).FirstOrDefault();
            if (custAdr == null)
            {
                custAdr = (Address)details.Customer.BillingAddress.Clone(); ;

            }
            details.BillingAddress = (Address)custAdr.Clone();
            if (processPaymentRequest.ShippingAddressId > 0)
            {
                var address = new Address();
                var addresses = details.Customer.Addresses;
                if (!addresses.Any())
                {
                    addresses = _customerService.GetCustomerById(details.Customer.Id).Addresses;
                }
                if (addresses.Any())
                {
                    address = details.Customer.Addresses.FirstOrDefault(a => a.Id == processPaymentRequest.ShippingAddressId);
                    if (address != null)
                    {
                        details.ShippingAddress = (Address)address.Clone();
                        details.Customer.ShippingAddress = (Address)address.Clone();
                    }
                }
            }
            if (details.BillingAddress.Country != null && !details.BillingAddress.Country.AllowsBilling)
            ...
            details.CheckoutAttributesXml = _genericAttributeService.GetAttribute<string>(details.Customer, NopCustomerDefaults.CheckoutAttributes, processPaymentRequest.StoreId);
            details.CheckoutAttributeDescription = _checkoutAttributeFormatter.FormatAttributes(details.CheckoutAttributesXml, details.Customer);
            // Quick Order Processng bc its not part of Cart
            if (orderItems != null && orderItems.Count > 0)
                details.Customer.ShoppingCartItems.Clear();
            foreach (OrderItem oItem in orderItems)
            {
                // Bulk Order should clear all Cart Items                
                _shoppingCartService.AddToCart2(details.Customer, oItem.Product, ShoppingCartType.ShoppingCart, processPaymentRequest.StoreId, oItem.AttributesXml, oItem.UnitPriceExclTax, rentalStartDate: null, rentalEndDate: null, oItem.Quantity, addRequiredProducts: false, isQuickOrder: true);
            }
            //load shopping cart
            details.Cart = _shoppingCartService.GetShoppingCart(details.Customer, ShoppingCartType.ShoppingCart, processPaymentRequest.StoreId);
            foreach (var cartItem in details.Cart)
            {
                cartItem.IsQuickOrder = true;
            }
            if (!details.Cart.Any())
                throw new NopException("Cart is empty");
            //min totals validation
            ...
            if (_shoppingCartService.ShoppingCartRequiresShipping(details.Cart))
            {
                ...
                details.ShippingStatus = ShippingStatus.ShippingNotRequired;
            //details.Customer.ShippingAddress = 0;

            //LoadAllShippingRateComputationMethods
            var shippingRateComputationMethods = _shippingPluginManager.LoadActivePlugins(details.Customer, _storeContext.CurrentStore.Id);
            ...
            if (!orderTotal.HasValue)
                throw new NopException("Order total couldn't be calculated");
            orderTotal += details.PaymentAdditionalFeeInclTax;
            details.OrderDiscountAmount = orderDiscountAmount;
            details.RedeemedRewardPoints = redeemedRewardPoints;
            details.RedeemedRewardPointsAmount = redeemedRewardPointsAmount;
            details.AppliedGiftCards = appliedGiftCards;
        ...
        protected virtual Order SaveOrderDetails(ProcessPaymentRequest processPaymentRequest,
            ProcessPaymentResult processPaymentResult, PlaceOrderContainer details, OrderStatus orderStatus = OrderStatus.Pending)
        {
            var order = new Order
            {
                ...
                SalemanId = details.SalemanId,
                ReferneceNo = processPaymentRequest.ReferneceNo,
                ActualShippingCost = details.OrderShippingTotalInclTax
            };
        ...
        /// <param name="order">Order</param>
        protected virtual void MoveShoppingCartItemsToOrderItems(PlaceOrderContainer details, Order order, bool isQuickOrder = false)
        {
            foreach (var sc in details.Cart)
            {
                ...
                //inventory
                if (isQuickOrder && (order.OrderStatus == OrderStatus.Pending || order.OrderStatus == OrderStatus.Complete || order.OrderStatus == OrderStatus.Processing))
        ...
        public virtual void CheckOrderStatus(Order order)
        {
            ...
            _orderService.UpdateOrder(order, true);
            if (order.SalemanId != 0 && order.StoreId == (int)OrderStatus.Complete)
            {
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult PlaceQuickOrder(ProcessPaymentRequest processPaymentRequest, List<OrderItem> OrderItems = null, OrderStatus orderStatus = OrderStatus.Pending)
        {
            if (processPaymentRequest == null)
                throw new ArgumentNullException(nameof(processPaymentRequest));
            ...
            try
            {
                ...
                //prepare quick order details, using collection of Products here
                var details = this.PreparePlaceQuickOrderDetails(processPaymentRequest, OrderItems);
                var processPaymentResult = GetProcessPaymentResult(processPaymentRequest, details);
                ...
                if (processPaymentResult.Success)
                {
                    var order = SaveOrderDetails(processPaymentRequest, processPaymentResult, details, orderStatus: orderStatus);
                    result.PlacedOrder = order;

                    //move shopping cart items to order items
                    MoveShoppingCartItemsToOrderItems(details, order, true);
        ...
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <returns>Place order result</returns>
        public virtual PlaceOrderResult UpdateQuickOrder(Order order, List<OrderItem> OrderItems = null, OrderStatus OldStatus = OrderStatus.Processing)
        {

            if (_taxService.IsTaxExempt(null, order.Customer))
            {
                foreach (var item in OrderItems)
                {
                    if (item.PriceInclTax != item.PriceExclTax)
                        item.PriceInclTax = item.PriceExclTax;
                }
            }
            var result = new PlaceOrderResult();
            bool isStatusPromoted = ((OldStatus == OrderStatus.Offer || OldStatus == OrderStatus.BackOrder) && (order.OrderStatus == OrderStatus.Processing || order.OrderStatus == OrderStatus.Pending || order.OrderStatus == OrderStatus.Complete)) ? true : false;
            try
            {
                if (order == null)
                    throw new Exception("Order GUID is not generated");
                ...
                if (OrderItems == null || OrderItems.Count == 0)
                    throw new Exception("Order items empty, for bulk order need items");
                //************************************ Update Product Items only when there is change in items********************************/
                if (isStatusPromoted || (order.OrderItems.Count != OrderItems.Count || order.OrderItems.Select(t => t.Quantity).Sum() != OrderItems.Select(x => x.Quantity).Sum() || order.OrderItems.Select(q => Convert.ToInt32(q.PriceExclTax)).Sum() != OrderItems.Select(k => Convert.ToInt32(k.PriceExclTax)).Sum()) || order.CustomShippingCharges == null)
                {
                    ...
                    var OldProductQtys = (from itms in order.OrderItems
                                          select new
                                          {
                    ...
                    List<OrderItem> OItems = new List<OrderItem>(order.OrderItems.ToList().ToArray());
                    ...
                    foreach (OrderItem item in OrderItems)
                    {
                        ...
                        var orderItem = new OrderItem
                        {
                            ...
                            RentalEndDateUtc = item.RentalStartDateUtc
                        };
                        order.OrderItems.Add(orderItem);
                        //   _orderService.UpdateOrder(order);
                        //inventory
                        if ((OldStatus == OrderStatus.Offer || OldStatus == OrderStatus.BackOrder) && (order.OrderStatus == OrderStatus.Processing || order.OrderStatus == OrderStatus.Pending || order.OrderStatus == OrderStatus.Complete))
                        {
                        ...
                        else if (order.OrderStatus == OrderStatus.Processing || order.OrderStatus == OrderStatus.Pending || order.OrderStatus == OrderStatus.Complete)
                        {
                            // Get Old Quantity & then take difference
                            int OldQuantity = 0;
                            if (OldProductQtys.Where(x => x.ProductId == product.Id && x.OrderId == order.Id).FirstOrDefault() != null)
                            {
                                OldQuantity = OldProductQtys.Where(x => x.ProductId == product.Id && x.OrderId == order.Id).Select(t => t.Quantity).Sum();
                            }
                            _productService.AdjustInventory(product, -(orderItem.Quantity - OldQuantity), orderItem.AttributesXml,
                            string.Format(_localizationService.GetResource("Admin.StockQuantityHistory.Messages.EditOrder"), order.Id));
                        }
        ...
        /// <param name="order">Order</param>
        /// <returns>A value indicating whether order can be marked as paid</returns>
        public virtual bool CanMarkOrderAsUnPaid(Order order)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));
        ...
        /// </summary>
        /// <param name="order">Order</param>
        public virtual void MarkOrderAsUnPaid(Order order)
        {
            if (order == null)
                throw new ArgumentNullException(nameof(order));

            if (!CanMarkOrderAsUnPaid(order))
                throw new NopException("You can't mark this order as paCanMarkOrderAsPaidid");
            ...
            //add a note
            AddOrderNote(order, "Order has been marked as unpaid/pending");
