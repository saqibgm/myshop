    ...
    public partial class OrderTotalCalculationService : IOrderTotalCalculationService
    {
                ...
                if (!IsFreeShipping(restoredCart, _shippingSettings.FreeShippingOverXIncludingTax ? subTotalInclTax : subTotalExclTax))
                {
                    ...
                    if (!string.IsNullOrEmpty(updatedOrder.ShippingRateComputationMethodSystemName))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                if (updatedOrder.CustomShippingCharges != null)
                                {
                                    shippingTotal = updatedOrder.CustomShippingCharges.Value;
                                }
                                else
                                {
                        ...
                        if (updateOrderParameters.PickupPoint == null)
                        {
                            ...
                            if (shippingRateComputationMethods.Any())
                            {
                                ...
                                if (shippingOptionsResponse.Success)
                                {
                                    ...
                                    shippingTotal = updatedOrder.CustomShippingCharges != null ? updatedOrder.CustomShippingCharges.Value : shippingOption.Rate;
                                }
                                else
                                    updateOrderParameters.Warnings.AddRange(shippingOptionsResponse.Errors);
                            }
            ...
            out decimal taxRate, out List<DiscountForCaching> appliedDiscounts)
        {
            ...
            if (customer != null)
            {
                ...
                if (shippingOption == null && cart[0].IsQuickOrder)

                {
                    var shippingRateComputationMethodSystemName = "Shipping.FixedByWeightByTotal";
                    var selectedName = "Standard";
                    var shippingOptions = _shippingService.GetShippingOptions(cart, customer.ShippingAddress, customer, shippingRateComputationMethodSystemName, _storeContext.CurrentStore.Id).ShippingOptions.ToList();
                    shippingOption = shippingOptions.Find(so => !string.IsNullOrEmpty(so.Name) && so.Name.Equals(selectedName, StringComparison.InvariantCultureIgnoreCase));
                    //if (shippingOption != null)
