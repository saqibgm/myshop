...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        protected virtual string[] GetPictureURLs(Product product)
        {
            //pictures (up to 3 pictures)
            string picture1 = null;
            string picture2 = null;
            ...
            for (var i = 0; i < pictures.Count; i++)
            {
                var picUrl = _pictureService.GetPictureUrl(pictures[i]);
                switch (i)
                {
                    case 0:
                        picture1 = picUrl;
        ...
        public virtual string ExportProductsToXml(IList<Product> products)
        {
            ...
            foreach (var product in products)
            {
                ...
                xmlWriter.WriteString("Article No", product.Id + 10000);
                ...
                xmlWriter.WriteString("Manufacturer-Code", product.ManufacturerPartNumber);
                xmlWriter.WriteString("MinPrice", product.MinPrice.ToString("0.00") );
                var tierPriceA = product.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.A);
                decimal priceA = tierPriceA == null ? 0 : tierPriceA.Price;
                xmlWriter.WriteString("Price-A", priceA.ToString("0.00"));
                var tierPriceB = product.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.B);
                decimal priceB = tierPriceB == null ? 0 : tierPriceB.Price;
                xmlWriter.WriteString("Price-B", priceB.ToString("0.00"));
                var tierPriceC = product.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.C);
                decimal priceC = tierPriceC == null ? 0 : tierPriceC.Price;
                xmlWriter.WriteString("Price-C", priceC.ToString("0.00"));
                xmlWriter.WriteString("Stock-Quantity ", product.StockQuantity);


                //    xmlWriter.WriteString("ProductId", product.Id);product.MinPrice
                ...
                //    xmlWriter.WriteString("MinPrice", product.OldPrice, IgnoreExportPoductProperty(p => p.MinPrice));
            ...
            xmlWriter.WriteEndDocument();
            xmlWriter.Close();
            string csvStr = ConvertToCSV(stringWriter.ToString());
            return csvStr;
            //return stringWriter.ToString();
        }

        public virtual string ExportProductsToCSVForCustomer(IList<Product> products)
        {
            var sb = new StringBuilder();
            var stringWriter = new StringWriter(sb);
            var xmlWriter = new XmlTextWriter(stringWriter);
            ...
            foreach (var product in products)
            {
                ...
                if(_workContext.CurrentCustomer.CustomerRoles.Any(p => p.Id == (int)ProductPriceTier.A))
                {
                    var tierPrice = product.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.A);
                    price = tierPrice == null ? 0 : tierPrice.Price;
                }
                else if (_workContext.CurrentCustomer.CustomerRoles.Any(p => p.Id == (int)ProductPriceTier.B))
                {
                    var tierPrice = product.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.B);
                    price = tierPrice == null ? 0 : tierPrice.Price;
                }
                else if (_workContext.CurrentCustomer.CustomerRoles.Any(p => p.Id == (int)ProductPriceTier.C))
                {
                    var tierPrice = product.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.C);
                    price = tierPrice == null ? 0 : tierPrice.Price;
                }
                xmlWriter.WriteString("Price", price.ToString("0.00"));
                xmlWriter.WriteString("Stock-Quantity ", product.StockQuantity);
                xmlWriter.WriteEndElement();
            }
            //xmlWriter.WriteEndElement();
        ...
        protected string ConvertToCSV(string xmlStr)
        {

            DataSet ds = new DataSet();
            xmlStr = xmlStr.Replace(" >",">");
            xmlStr = xmlStr.Replace(" No>","No>");
            ds.ReadXml(new StringReader(xmlStr));

            StringBuilder sb = new StringBuilder();
            IEnumerable<string> columnNames = ds.Tables[1].Columns.Cast<DataColumn>().SkipLast(1).
                                              Select(column => column.ColumnName);
            sb.AppendLine(string.Join(",", columnNames));

            foreach (DataRow row in ds.Tables[1].Rows)
            {
                IEnumerable<string> fields = row.ItemArray.SkipLast(1).Select(field =>
        ...
        public virtual byte[] ExportProductsToXlsx(IEnumerable<Product> products)
        {
            ...
            var properties = new[]
            {
                new PropertyByName<Product>("Article-No", p => p.Id+10000),
                new PropertyByName<Product>("Name", p => p.Name),
                new PropertyByName<Product>("Description", p => p.ShortDescription),
                ...
                new PropertyByName<Product>("MinPrice", p => p.MinPrice.ToString("0.00")),
                new PropertyByName<Product>("Price A", p => (p.TierPrices.Count>0?p.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.A).Price.ToString("0.00"):"")),
                new PropertyByName<Product>("Price B", p => (p.TierPrices.Count>1?p.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.B).Price.ToString("0.00"):"")),
                new PropertyByName<Product>("Price C", p => (p.TierPrices.Count>2?p.TierPrices.FirstOrDefault(t => t.CustomerRoleId == (int)ProductPriceTier.C).Price.ToString("0.00"):"")),
                ...
                new PropertyByName<Product>("MinPrice", p => p.OldPrice, IgnoreExportPoductProperty(p => p.MinPrice)),
