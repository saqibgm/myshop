...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        public virtual IList<Product> GetWeclappNewProducts(int noOfProducts, DateTime timeAfter, bool retryFailed = false)
        {
            var products = _productRepository.Table.OrderBy(i => i.Id).Where(o => string.IsNullOrEmpty(o.Wid) || (retryFailed && o.Wid == "-1") || (o.WSynced==false));
            //var products = _productRepository.Table.OrderBy(i => i.Id).Where(o => string.IsNullOrEmpty(o.Wid) || (retryFailed && o.Wid == "-1") || (o.UpdatedOnUtc > timeAfter));
            if (noOfProducts > 0)
        ...
        public virtual void UpdateProduct(Product product)
        {
            ...
            product.WSynced = product.WSynced == null ? true : false;
            //update
        ...
        {
            return SearchProducts2(out var _, false,
                pageIndex, pageSize, categoryIds, manufacturerId, articleNo: null,
                ...
                orderBy, showHidden, overridePublished, null);
        }
        ...
        /// <param name="categoryIds">Category identifiers</param>
        /// <param name="manufacturerId">Manufacturer identifier; 0 to load all records</param>
        /// <param name="articleNo">articleNo</param>
        /// <param name="storeId">Store identifier; 0 to load all records</param>
        /// <param name="vendorId">Vendor identifier; 0 to load all records</param>
        /// <param name="warehouseId">Warehouse identifier; 0 to load all records</param>
        /// <param name="productType">Product type; 0 to load all records</param>
        ...
        public virtual IPagedList<Product> SearchProducts(
            ...
            bool? overridePublished = null)
        {
            return SearchProducts2(out filterableSpecificationAttributeOptionIds, loadFilterableSpecificationAttributeOptionIds,
                pageIndex, pageSize, categoryIds, manufacturerId, articleNo: null,
                storeId, vendorId, warehouseId,
                productType, visibleIndividuallyOnly, markedAsNewOnly, featuredProducts,
                priceMin, priceMax, productTagId, keywords, searchDescriptions, searchManufacturerPartNumber, searchSku,
        ...
        /// </param>
        /// <returns>Products</returns>
        public virtual IPagedList<Product> SearchProducts2(
            int pageIndex = 0,
            int pageSize = int.MaxValue,
            IList<int> categoryIds = null,
            int manufacturerId = 0,
            ...
            ProductSortingEnum orderBy = ProductSortingEnum.Position,
            bool showHidden = false,
            bool? overridePublished = null, bool? overrideNewStockComing = null)
        {
            return SearchProducts2(out var _, false,
                pageIndex, pageSize, categoryIds, manufacturerId, articleNo,
                storeId, vendorId, warehouseId,
                productType, visibleIndividuallyOnly, markedAsNewOnly, featuredProducts,
                priceMin, priceMax, productTagId, keywords, searchDescriptions, searchManufacturerPartNumber, searchSku,
                searchProductTags, languageId, filteredSpecs,
                orderBy, showHidden, overridePublished, overrideNewStockComing);
        }
            ...
            bool? overridePublished = null, bool? overrideNewStockComing = null)
        {
            ...
            var pArticleNo = _dataProvider.GetStringParameter("ArticleNo", articleNo);
            ...
            var pOverrideNewStockComing = _dataProvider.GetBooleanParameter("OverrideNewStockComing", overrideNewStockComing);
            ...
            var reservedProducts = _orderItemRepository.Table.Where(i => productIDs.Contains(i.ProductId) && (i.Order.OrderStatusId == (int)OrderStatus.Pending || i.Order.OrderStatusId == (int)OrderStatus.Processing) && !i.Order.Deleted).GroupBy(i => i.ProductId).ToDictionary(i => i.Key, i => i.Sum(oi => oi.Quantity));
            foreach (var product in products)
            {
                if (reservedProducts.ContainsKey(product.Id))
                {
                    product.StockReserved = reservedProducts[product.Id];
                }
            }
            //return products
        ...
        /// <param name="quantity">Quantity</param>
        /// <returns>Tier price</returns>
        public virtual List<TierPrice> GeCustomerTierPrices(Customer customer)
        {
            if (customer == null)
                return null;
            var roleIDs = customer.GetCustomerRoleIds();
            //get actual tier prices
            var tierPrices = _tierPriceRepository.Table.Where(t => roleIDs.Contains(t.CustomerRoleId.Value)).ToList();
