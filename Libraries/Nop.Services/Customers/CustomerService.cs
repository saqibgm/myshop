...
namespace Nop.Services.Customers
{
    ...
    public partial class CustomerService : ICustomerService
    {
        ...
        private readonly IRepository<CustomerCommission> _customerCommissionRepository;
        ...
        public CustomerService(CustomerSettings customerSettings,
            ...
            IRepository<CustomerCommission> customerCommissionRepository,
            IRepository<Order> orderRepository)
        {
            ...
            _customerCommissionRepository = customerCommissionRepository;
        ...
        public virtual IPagedList<Customer> GetAllCustomers(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
            ...
            int dayOfBirth = 0, int monthOfBirth = 0, string id = null,
        ...
        /// <param name="getOnlyTotalCount">A value in indicating whether you want to load only total number of records. Set to "true" if you don't want to load data from database</param>
        /// <returns>Customers</returns>
        public virtual IPagedList<Customer> GetAllCustomers2(DateTime? createdFromUtc = null, DateTime? createdToUtc = null,
            int affiliateId = 0, int vendorId = 0, int[] customerRoleIds = null,
            string email = null, string username = null, string firstName = null, string lastName = null,
            int dayOfBirth = 0, int monthOfBirth = 0, string id = null,
            string company = null, string phone = null, string zipPostalCode = null, string ipAddress = null,
            int pageIndex = 0, int pageSize = int.MaxValue, bool getOnlyTotalCount = false, bool itemsInCart = false,
            int customerStatus = 0, string city = null, string country = null, string salesmanid = null)
        {
            var query = _customerRepository.Table;
            if (createdFromUtc.HasValue)
                query = query.Where(c => createdFromUtc.Value <= c.CreatedOnUtc);
            ...
            if (!string.IsNullOrWhiteSpace(firstName))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z =>
                    (z.Attribute.KeyGroup == nameof(Customer) && z.Attribute.Key == NopCustomerDefaults.FirstNameAttribute && z.Attribute.Value.Contains(firstName)) ||
                    (z.Attribute.KeyGroup == nameof(Customer) && z.Attribute.Key == NopCustomerDefaults.LastNameAttribute && z.Attribute.Value.Contains(firstName)))
                    .Select(z => z.Customer);
            }
            ...
            if (!string.IsNullOrWhiteSpace(city))
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == NopCustomerDefaults.CityAttribute &&
                                z.Attribute.Value.Contains(city))
                    .Select(z => z.Customer);
            }
            //search by city
            if (!string.IsNullOrWhiteSpace(country) && country != "0")
            {
                query = query
                    .Join(_gaRepository.Table, x => x.Id, y => y.EntityId, (x, y) => new { Customer = x, Attribute = y })
                    .Where(z => z.Attribute.KeyGroup == nameof(Customer) &&
                                z.Attribute.Key == NopCustomerDefaults.CountryIdAttribute &&
                                z.Attribute.Value == country)
                    .Select(z => z.Customer);
            }
            //date of birth is stored as a string into database.
            //we also know that date of birth is stored in the following format YYYY-MM-DD (for example, 1983-02-18).
            ...
            if (!string.IsNullOrWhiteSpace(salesmanid))
            {
                if (salesmanid == "-1")
                {
                    query = query.Where(w => w.TeamMemberOfSaleman == null || w.TeamMemberOfSaleman == 0);
                }
                else
               if (salesmanid == "0")
                {
                    //query = query.Where(w => w.TeamMemberOfSaleman == 0);
                }
                else
                {
                    query = query.Where(w => w.TeamMemberOfSaleman.ToString() == salesmanid);
                }
            }
            query = query.OrderByDescending(c => c.CreatedOnUtc);
        ...
        /// <summary>
        /// Calculatw salesman commission by salemanId and month
        /// </summary>
        /// <param name="salemanId">Saleman Id</param>
        /// <param name="month">month</param>
        /// <returns>Order</returns>
        public virtual CustomerCommission CalculateSalemanCommission(int salemanId, DateTime month)
        {
            var customerCommission = new CustomerCommission();
            if (salemanId != 0)
            {
                var commissionMonth = new DateTime(month.Year, month.Month, 1);
                customerCommission = _customerCommissionRepository.Table.FirstOrDefault(c => c.CustomerId == salemanId && c.CommissionMonth == commissionMonth);
                if (customerCommission == null)
                {
                    customerCommission = new CustomerCommission();
                    customerCommission.CreatedOnUtc = DateTime.Now;
                    customerCommission.Active = true;
                    customerCommission.Deleted = false;
                    customerCommission.CommissionMonth = commissionMonth;
                    var saleman = _customerRepository.Table.FirstOrDefault(c => c.Id == salemanId);
                    if (saleman != null)
                    {

                        customerCommission.CustomerId = salemanId;
                        customerCommission.BasicSalary = saleman.BasicSalary;
                        customerCommission.CommissionMinTargert = saleman.CommissionMinTargert;
                        customerCommission.CommissionPercentage = saleman.CommissionPercentage;
                    }
                }
                var query = _orderRepository.Table;
                query = query.Where(o => o.SalemanId == salemanId && o.CreatedOnUtc.Month == commissionMonth.Month && o.CreatedOnUtc.Year == commissionMonth.Year);
                query = query.OrderByDescending(o => o.CreatedOnUtc);
                var orders = query.ToList();

                customerCommission.TotalOrders = orders.Count();
                customerCommission.TotalSale = orders.Sum(o => o.OrderTotal);
                customerCommission.TotalProfit = orders.Sum(o => o.Profit == null ? 0 : o.Profit.Value);
                customerCommission.TotalSalary = customerCommission.BasicSalary == null ? 0 : customerCommission.BasicSalary.Value;
                if (customerCommission.TotalProfit > customerCommission.CommissionMinTargert)
                {
                    customerCommission.TotalCommission = (customerCommission.TotalProfit - customerCommission.CommissionMinTargert ?? 0) * ((customerCommission.CommissionPercentage ?? 0) / 100);
                    customerCommission.TotalSalary += customerCommission.TotalCommission;
                }
                customerCommission.LastActivityDateUtc = DateTime.Now;
                if (customerCommission.Id == 0)
                    _customerCommissionRepository.Insert(customerCommission);
                else
                    _customerCommissionRepository.Update(customerCommission);
            }
        ...
        /// <param name="month">month</param>
        /// <returns>Order</returns>
        public virtual CustomerCommission ReCalculateSalemanCommission(int commissionId)
        {
            var salemanCommission = GetSalemanCommissionById(commissionId);
            if (salemanCommission != null && salemanCommission.CommissionMonth.AddMonths(1).Month >= DateTime.Now.Month)
            {
                var salemanId = salemanCommission.CustomerId;
                var commissionMonth = new DateTime(salemanCommission.CommissionMonth.Year, salemanCommission.CommissionMonth.Month, 1);
                var saleman = _customerRepository.Table.FirstOrDefault(c => c.Id == salemanId);
                if (saleman != null)
                {

                    salemanCommission.CustomerId = salemanId;
                    salemanCommission.BasicSalary = saleman.BasicSalary;
                    salemanCommission.CommissionMinTargert = saleman.CommissionMinTargert;
                    salemanCommission.CommissionPercentage = saleman.CommissionPercentage;
                }
                var query = _orderRepository.Table;
                query = query.Where(o => o.SalemanId == salemanId && o.CreatedOnUtc.Month == commissionMonth.Month && o.CreatedOnUtc.Year == commissionMonth.Year && o.OrderStatusId == (int)OrderStatus.Complete && !o.Deleted);
                query = query.OrderByDescending(o => o.CreatedOnUtc);
                var orders = query.ToList();

                salemanCommission.TotalOrders = orders.Count();
                salemanCommission.TotalSale = orders.Sum(o => o.OrderTotal);
                salemanCommission.TotalProfit = orders.Sum(o => o.Profit == null ? 0 : o.Profit.Value);
                salemanCommission.TotalSalary = salemanCommission.BasicSalary == null ? 0 : salemanCommission.BasicSalary.Value;
                if (salemanCommission.TotalProfit > salemanCommission.CommissionMinTargert)
                {
                    salemanCommission.TotalCommission = salemanCommission.TotalProfit > salemanCommission.CommissionMinTargert ? (salemanCommission.TotalProfit * ((salemanCommission.CommissionPercentage ?? 0) / 100)) : salemanCommission.BasicSalary ?? 0;
                    salemanCommission.TotalSalary = salemanCommission.TotalCommission;
                }
                salemanCommission.LastActivityDateUtc = DateTime.Now;
                _customerCommissionRepository.Update(salemanCommission);
            }
        ...
        /// <summary>
        /// Get salesman commission by id
        /// </summary>
        /// <param name="commission id">commission Id</param>
        /// <returns>Order</returns>
        public virtual CustomerCommission GetSalemanCommissionById(int id)
        {
            var customerCommission = _customerCommissionRepository.Table.FirstOrDefault(c => c.Id == id);
            return customerCommission;
        }
        /// <summary>
        /// Get salesman commission by salemanId and month
        /// </summary>
        /// <param name="salemanId">Saleman Id</param>
        /// <param name="month">month</param>
        /// <returns>Order</returns>
        public virtual CustomerCommission GetSalemanCommissionByMonth(int salemanId, DateTime month)
        {
            var customerCommission = _customerCommissionRepository.Table.FirstOrDefault(c => c.CustomerId == salemanId && c.CommissionMonth == month);
            return customerCommission;
        }
        /// <summary>
        /// Get salesman commission by salemanId and month
        ...
        /// <param name="month">month</param>
        /// <returns>Order</returns>
        public virtual IPagedList<CustomerCommission> GetSalemanCommissions(int salemanId, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _customerCommissionRepository.Table.Where(c => c.CustomerId == salemanId).OrderByDescending(c => c.CommissionMonth);
            return new PagedList<CustomerCommission>(query, pageIndex, pageSize);
        }
        ...
        public virtual IPagedList<Customer> GetOnlineCustomersForSalesMan(DateTime lastActivityFromUtc, int salesmanId, int[] customerRoleIds, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = _customerRepository.Table;
            query = query.Where(c => c.TeamMemberOfSaleman == salesmanId);
            query = query.Where(c => lastActivityFromUtc <= c.LastActivityDateUtc);
            query = query.Where(c => !c.Deleted);
            if (customerRoleIds != null && customerRoleIds.Length > 0)
                query = query.Where(c => c.CustomerCustomerRoleMappings.Select(mapping => mapping.CustomerRoleId).Intersect(customerRoleIds).Any());
        ...
        public virtual IPagedList<Customer> GetCustomersWithShoppingCarts(ShoppingCartType? shoppingCartType = null,
                int storeId = 0, int? productId = null, int salesmanId = 0,
                ...
                int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (salesmanId != 0)
            {
                items = items.Where(item => item.Customer.TeamMemberOfSaleman == salesmanId);
            }
            //filter by type
            ...
            customers = customers.Where(item => item.TeamMemberOfSaleman == salesmanId);

            //filter customers by billing country
            if (countryId > 0)
                customers = customers.Where(customer => customer.BillingAddress.CountryId == countryId);
            var customers2 = customers.AsEnumerable().Where(c => c.IsCustomer(true));

            //get customers with shopping carts
            var custIDs = items.GroupBy(item => item.CustomerId)
                .Select(cart => new { CustomerId = cart.Key, CreatedOnUtc = cart.Max(item => item.UpdatedOnUtc) })
            ...
            //var customersWithCarts = custIDs.Join(customers2, id => id, customer => customer.Id, (id, customer) => customer);
            var cartCustomer = _customerRepository.Table.Where(c => custIDs.Contains(c.Id)).OrderBy(l => custIDs.IndexOf(l.Id)).ToList();
            //.OrderByDescending(c => c.Id).ToList();
            //var carts = customersWithCarts.ToList();
            return new PagedList<Customer>(cartCustomer, pageIndex, pageSize);
        }
        ...
        public virtual IList<Customer> GetWeclappNewCustomers(int noOfCustomers, DateTime timeAfter, bool retryFailed = false)
        {
            var customerRoles = new int[] { 7, 8, 9, 12 };

            var customers = _customerCustomerRoleMappingRepository.Table.Where(r => customerRoles.Contains(r.CustomerRoleId)).Select(c => c.Customer);
            //customers = customers.OrderByDescending(i => i.Id).Where(o => string.IsNullOrEmpty(o.Wid) || (retryFailed && o.Wid == "-1") || (o. > timeAfter));
            customers = customers.OrderByDescending(i => i.Id).Where(o => string.IsNullOrEmpty(o.Wid) || (retryFailed && o.Wid == "-1") || (o.WSynced==false));
            if (noOfCustomers > 0)
        ...
        public virtual void UpdateCustomer(Customer customer)
        {
            ...
            customer.WSynced = customer.WSynced == null ? true : false;
        ...
        /// </summary>
        /// <param name="customer">Customer</param>
        public virtual void UpdateCustomers(List<Customer> customers)
        {
        ...
        /// <param name="customerRoleId">Customer role identifier</param>
        /// <returns>Customer role</returns>
        public virtual List<Customer> GetAllCustomerByRoles(List<int> customerRoles)
        {
            if (customerRoles.Count() <= 0)
                return null;

            return _customerCustomerRoleMappingRepository.Table.Where(r => customerRoles.Contains(r.CustomerRoleId)).Select(c => c.Customer).ToList();
        }
        ...
        /// <param name="systemName">Customer role system name</param>
        /// <returns>Customer role</returns>
        public virtual List<CustomerRole> GetCustomerRolesByName(string systemName)
        {
            if (string.IsNullOrWhiteSpace(systemName))
                return null;
            ...
            return _cacheManager.Get(key, () =>
            {
                var query = from cr in _customerRoleRepository.Table
                            orderby cr.Id
                            where cr.SystemName.Contains(systemName)
                            select cr;
