...
namespace Nop.Services.Common
{
    ...
    public partial class GenericAttributeService : IGenericAttributeService
    {
        ...
        /// <param name="keyGroup">Key group</param>
        /// <returns>Get attributes</returns>
        public virtual IList<GenericAttribute> GetFullNamesByKey(string keyGroup, List<int> IDs)
        {
            var akey = string.Format(NopCommonDefaults.GenericAttributeCacheKey, NopCustomerDefaults.CompanyAttribute, keyGroup);
            return _cacheManager.Get(akey, () =>
            {
                var query = from ga in _genericAttributeRepository.Table
                            where (ga.Key == NopCustomerDefaults.CompanyAttribute && ga.KeyGroup == keyGroup) ||
                            (IDs.Contains(ga.EntityId) && ga.Key == NopCustomerDefaults.FirstNameAttribute) ||
                            (IDs.Contains(ga.EntityId) && ga.Key == NopCustomerDefaults.LastNameAttribute)
                            select ga;
                var attributes = query.ToList();
                return attributes;
            });
