    ...
    public partial class CheckoutController : BasePublicController
    {
        ...
        public virtual IActionResult Index()
        {
            ...
            foreach (var sci in cart)
            {
                ...
                if (sciWarnings.Any())
                {
                    _notificationService.ErrorNotification(_localizationService.GetResource("Checkout.RemoveOutOfStockProducts"));
                    return RedirectToRoute("ShoppingCart");
                }
            }
        ...
        public virtual IActionResult OpcConfirmOrder()
        {
            try
            {
                ...
                processPaymentRequest.StoreId = _storeContext.CurrentStore.Id;
                processPaymentRequest.CustomerId = _workContext.CurrentCustomer.Id;
                processPaymentRequest.SalesmanId = _workContext.CurrentCustomer.TeamMemberOfSaleman;
                processPaymentRequest.PaymentMethodSystemName = _genericAttributeService.GetAttribute<string>(_workContext.CurrentCustomer,
                    NopCustomerDefaults.SelectedPaymentMethodAttribute, _storeContext.CurrentStore.Id);
                HttpContext.Session.Set<ProcessPaymentRequest>("OrderPaymentInfo", processPaymentRequest);
                var placeOrderResult = _orderProcessingService.PlaceOrder(processPaymentRequest);
