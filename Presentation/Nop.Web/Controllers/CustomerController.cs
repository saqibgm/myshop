...
namespace Nop.Web.Controllers
{
    public partial class CustomerController : BasePublicController
    {
        ...
        [CheckAccessPublicStore(true)]
        //[HttpPost]
        public virtual IActionResult ExportCSVAll()
        {
            //a customer should have access only to his products
            if (_workContext.CurrentCustomer == null || !_workContext.CurrentCustomer.IsExportAllowed)
            {
            ...
            try
            {
                var xml = _exportManager.ExportProductsToCSVForCustomer(products);

                return File(Encoding.UTF8.GetBytes(xml), MimeTypes.ApplicationXml, "Createl Products.csv");
            }
            catch (Exception exc)
            {
        ...
        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult ContactInfo()
        {
            if (!_workContext.CurrentCustomer.IsRegistered() || _workContext.CurrentCustomer.TeamMemberOfSaleman == 0)
                return Challenge();
            ...
            // Get Saleman Id           
            model.SalemanId = _workContext.CurrentCustomer.TeamMemberOfSaleman;// Convert.ToInt32(Request.Form["salemanId"].ToString()); 
            //prepare model
        ...
        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult CustomerProducts()
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return Challenge();

            var model = _customerModelFactory.PrepareCustomerProductListModel();
            return View(model);
        }
        [HttpPost]
        [PublicAntiForgery]
        ...
        [HttpsRequirement(SslRequirement.Yes)]
        public virtual IActionResult BusinessCert()
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return Challenge();

            var model = new CustomerBusinessCertModel();
            model.BusinessCertId = _workContext.CurrentCustomer.BusinessCertId;
            model.IsCustomer = _workContext.CurrentCustomer.IsCustomer();
            if (model.BusinessCertId > 0)
                model.BusinessCertUrl = Url.Action("GetFileUploadById", "Download", new { downloadId = model.BusinessCertId });
            return View(model);
        }

        [HttpPost, ActionName("BusinessCert")]
        [PublicAntiForgery]
        [FormValueRequired("upload-business-cert")]
        public virtual IActionResult UploadBusinessCert(CustomerBusinessCertModel model, IFormFile uploadedFile)
        {

            var customer = _customerService.GetCustomerById(model.BusinessCertId);
            ...
            if (uploadedFile == null || string.IsNullOrEmpty(uploadedFile.FileName))
            {
                ...
                {
                    success = false,
                    message = string.Format(_localizationService.GetResource("Account.BusinessCert.MaximumUploadedFileSize"), fileMaxSize),
            ...
            model = _customerModelFactory.PrepareCustomerBusinessCertModel(model);
            return View(model);
        }
        ...
        public virtual IActionResult RemoveBusinessCert(CustomerBusinessCertModel model)
        {
            if (!_workContext.CurrentCustomer.IsRegistered())
                return Challenge();
            ...
            _customerService.UpdateCustomer(customer);
            return RedirectToRoute("CustomerBusinessCert");
        }
