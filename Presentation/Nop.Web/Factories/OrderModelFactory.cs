    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        public virtual CustomerOrderListModel PrepareCustomerOrderListModel()
        {
            ...
            foreach (var order in orders)
            {
                ...
                if ((order.OrderStatus == OrderStatus.Complete || order.OrderStatus == OrderStatus.Pending || order.OrderStatus == OrderStatus.Processing) &&
                    (order.PaymentStatus == Core.Domain.Payments.PaymentStatus.Pending))
                    balance -= orderTotalInCustomerCurrency;
            }

            model.OurStandingBalance = _priceFormatter.FormatPrice(balance, true, _workContext.WorkingCurrency.CurrencyCode, false, _workContext.WorkingLanguage);
