...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class BaseAdminModelFactory : IBaseAdminModelFactory
    {
        ...
        protected virtual void PrepareDefaultItem(IList<CustomerSalemanListItem> items, bool withSpecialDefaultItem, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));
            ...
            //insert this default item at first
            items.Insert(0, new CustomerSalemanListItem { Text = defaultItemText, Value = value, SalemanId = 0 });
        }
        ...
        public virtual void PreparePaymentTerms(IList<SelectListItem> items, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            ...
            items.Insert(1, new SelectListItem { Text = _localizationService.GetResource("Admin.Orders.PaymentTerm.7Days"), Value = "1" });
            items.Insert(2, new SelectListItem { Text = _localizationService.GetResource("Admin.Orders.PaymentTerm.14Days"), Value = "2" });
            items.Insert(3, new SelectListItem { Text = _localizationService.GetResource("Admin.Orders.PaymentTerm.COD"), Value = "3" });
            items.Insert(4, new SelectListItem { Text = _localizationService.GetResource("Admin.Orders.PaymentTerm.OSA"), Value = "4" });
            items.Insert(4, new SelectListItem { Text = _localizationService.GetResource("Admin.Orders.PaymentTerm.PrePay"), Value = "5" });
            //prepare available order statuses
            //var paymentTermsItems = PaymentTerms.Days7.ToSelectList(false);
            //foreach (var paymentTerm in paymentTermsItems)
            //{
            //    items.Add(paymentTerm);
            //}
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PrepareCustomersWRTRole(IList<SelectListItem> items, string role = null, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            ...
            //prepare available customer roles
            var roleIDs = _customerService.GetCustomerRolesByName(role).Select(r => r.Id).ToList();
            var availableCustomers = _customerService.GetAllCustomerByRoles(roleIDs).Distinct().ToList();
            var customersIDs = availableCustomers.Select(c => c.Id).Distinct().ToList();
            var namesList = _genericAttributeService.GetFullNamesByKey(role, customersIDs);
            //var availableCustomers = _customerService.GetAllCustomers();
            foreach (var customer in availableCustomers)
            {
                var fullName = "";
                string val = namesList.Where(a => a.EntityId == customer.Id && a.Key == NopCustomerDefaults.CompanyAttribute).FirstOrDefault()?.Value ?? "";
                if (!string.IsNullOrEmpty(val)) fullName += val + ", ";
                val = namesList.Where(a => a.EntityId == customer.Id && a.Key == NopCustomerDefaults.FirstNameAttribute).FirstOrDefault()?.Value ?? "";
                if (!string.IsNullOrEmpty(val)) fullName += val + " ";
                val = namesList.Where(a => a.EntityId == customer.Id && a.Key == NopCustomerDefaults.LastNameAttribute).FirstOrDefault()?.Value ?? "";
                if (!string.IsNullOrEmpty(val)) fullName += val;
                items.Add(new SelectListItem { Value = customer.Id.ToString(), Text = fullName });
            }// For Each
            PrepareDefaultItem(items, withSpecialDefaultItem, defaultItemText);
        }
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PrepareCustomersWRTRoleQuickOrder(IList<CustomerSalemanListItem> items, string needCustomersWithRole = null, int CurrentUserId = 0, int orderCustomerId = 0, string defaultItemText = null)
        {

            if (items == null)
                throw new ArgumentNullException(nameof(items));
            string customerName = string.Empty;
            var roleIDs = _customerService.GetCustomerRolesByName(needCustomersWithRole).Select(r => r.Id).ToList();
            var availableCustomers = _customerService.GetAllCustomerByRoles(roleIDs).Distinct().ToList();
           // var availableCustomers = _customerService.GetAllCustomerByRoles(roleIDs).Distinct().Where(c => orderCustomerId == 0 || c.Id == orderCustomerId).ToList();
            var customersIDs = availableCustomers.Select(c => c.Id).Distinct().ToList();

            var namesList = _genericAttributeService.GetFullNamesByKey(needCustomersWithRole, customersIDs);

            if (CurrentUserId > 0)
            {
                List<CustomerRole> Roles = _customerService.GetCustomerById(CurrentUserId).CustomerRoles.ToList();
                //prepare available customer roles
                if (Roles.Where(x => x.Name.ToLower().Contains(NopCustomerDefaults.CustomerRoleName.ToLower())).FirstOrDefault() != null && (needCustomersWithRole.ToLower().Contains(NopCustomerDefaults.CustomerRoleName.ToLower())))
                {
                    availableCustomers = availableCustomers.Where(x => x.Id == CurrentUserId && !x.IsGuest() && !x.IsAdmin()).ToList();
                }
                else if (Roles.Where(x => x.Name.ToLower().Contains(NopCustomerDefaults.CustomerRoleName.ToLower())).FirstOrDefault() != null && needCustomersWithRole.ToLower().Contains(NopCustomerDefaults.SalemanRoleName.ToLower()))
                {
                    var allCustomers = _customerService.GetAllCustomers();
                    availableCustomers = (from s in availableCustomers // outer sequence
                                          join al in allCustomers
                                          on s.Id equals al.TeamMemberOfSaleman // key selector
                                          where al.Id == CurrentUserId
                                          select s).ToList();

                }
                else if (Roles.Where(x => x.Name.ToLower().Contains(NopCustomerDefaults.SalemanRoleName.ToLower())).FirstOrDefault() != null && needCustomersWithRole.ToLower().Contains(NopCustomerDefaults.CustomerRoleName.ToLower()))
                {
                    availableCustomers = availableCustomers.Where(x => x.TeamMemberOfSaleman == CurrentUserId && !x.IsGuest() && !x.IsAdmin()).ToList();
                }
                else if (Roles.Where(x => x.Name.ToLower().Contains(NopCustomerDefaults.SalemanRoleName.ToLower())).FirstOrDefault() != null && needCustomersWithRole.ToLower().Contains(NopCustomerDefaults.SalemanRoleName.ToLower()))
                {
                    availableCustomers = availableCustomers.Where(x => x.Id == CurrentUserId && !x.IsGuest() && !x.IsAdmin()).ToList();
                }
            }
            ...
            foreach (var customer in availableCustomers)
            {
                var company = namesList.Where(a => a.EntityId == customer.Id && a.Key == NopCustomerDefaults.CompanyAttribute).FirstOrDefault();
                var usernName = (namesList.Where(a => a.EntityId == customer.Id && a.Key == NopCustomerDefaults.FirstNameAttribute).FirstOrDefault()?.Value + " " ?? "") +
                               (namesList.Where(a => a.EntityId == customer.Id && a.Key == NopCustomerDefaults.LastNameAttribute).FirstOrDefault()?.Value ?? "");
                var fullName = ((!string.IsNullOrEmpty(company?.Value)) ? company.Value + ", " : "") + usernName;
                items.Add(new CustomerSalemanListItem { Value = customer.Id.ToString(), Text = fullName, SalemanId = customer.TeamMemberOfSaleman });
            }// For Each
            PrepareDefaultItem(items, false, defaultItemText);
        }
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PrepareCustomers(IList<SelectListItem> items, string role = null, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            if (items == null)
                throw new ArgumentNullException(nameof(items));
            string customerName = string.Empty;
            ...
            var salesmanid = _workContext.CurrentCustomer.IsSaleman() ? _workContext.CurrentCustomer.Id : 0;
            if (salesmanid == 0)
            {
                availableCustomers = _customerService.GetAllCustomers().Where(x => x.IsRegistered()).ToList();
            }
            else
            {
                availableCustomers = _customerService.GetAllCustomers2(salesmanid: salesmanid.ToString()).Where(x => x.IsRegistered()).ToList();
            }
            foreach (var customer in availableCustomers)
            {
                if (customer != null)
                {
                    // User Customer Service to get through out system implemented name scheme
                    items.Add(new SelectListItem { Value = customer.Id.ToString(), Text = _customerService.GetCustomerFullName(customer) });
                }
                //insert special item for the default value
            }// For Each
            PrepareDefaultItem(items, withSpecialDefaultItem, defaultItemText);
        ...
        /// <param name="withSpecialDefaultItem">Whether to insert the first special item for the default value</param>
        /// <param name="defaultItemText">Default item text; pass null to use default value of the default item text</param>
        public virtual void PrepareSalesmanCustomers(IList<SelectListItem> items, string role = null, bool withSpecialDefaultItem = true, string defaultItemText = null)
        {
            var salesmanid = _workContext.CurrentCustomer.IsSaleman() ? _workContext.CurrentCustomer.Id : 0;
            if (items == null)
                throw new ArgumentNullException(nameof(items));
            string customerName = string.Empty;
            //prepare available customer roles
            var availableCustomers = _customerService.GetAllCustomers2(salesmanid: salesmanid.ToString()).Where(x => x.IsRegistered()).ToList();
            foreach (var customer in availableCustomers)
            {
                if (customer != null)
                {
