...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        public virtual ProductSearchModel PrepareProductSearchModel(ProductSearchModel searchModel)
        {
            ...
            //prepare "published" filter (0 - all; 1 - published only; 2 - unpublished only)
            searchModel.AvailableNewStockComingOptions.Add(new SelectListItem
            {
                Value = "0",
                Text = _localizationService.GetResource("Admin.Catalog.Products.List.NewStockComing.All")
            });
            searchModel.AvailableNewStockComingOptions.Add(new SelectListItem
            {
                Value = "1",
                Text = _localizationService.GetResource("Admin.Catalog.Products.List.NewStockComing.Yes")
            });
            searchModel.AvailableNewStockComingOptions.Add(new SelectListItem
            {
                Value = "2",
                Text = _localizationService.GetResource("Admin.Catalog.Products.List.NewStockComing.No")
            });
        ...
        public virtual ProductListModel PrepareProductListModel(ProductSearchModel searchModel)
        {
            ...
            var overrideNewStockComing = searchModel.SearchNewStockComingId == 0 ? null : (bool?)(searchModel.SearchNewStockComingId == 1);
            if (_workContext.CurrentVendor != null)
                searchModel.SearchVendorId = _workContext.CurrentVendor.Id;
            var categoryIds = new List<int>();
            if (searchModel.SearchCategoryIds != null && searchModel.SearchCategoryIds.Count > 0)
            {
                categoryIds.AddRange(searchModel.SearchCategoryIds);
            }
            else
            {
            ...
            //get products
            var products = _productService.SearchProducts2(showHidden: true,
                articleNo: searchModel.SearchArticleNo,
                ...
                overrideNewStockComing: overrideNewStockComing);
            ...
            var model = new ProductListModel().PrepareToGrid(searchModel, products, () =>
            {
                return products.Select(product =>
                {
                    ...
                    productModel.CategoriesText = string.Join(" | ", product.ProductCategories.Select(p => p.Category.Name).ToList());
                    productModel.SelectedCategoryIds = product.ProductCategories.Select(p => p.Category.Id).ToList();
                    if (product.TierPrices.Count > 0)
                    {
                        productModel.TierPriceA = product.TierPrices.FirstOrDefault(p => p.CustomerRoleId == (int)ProductPriceTier.A)?.Price ?? 0;
                        productModel.TierPriceB = product.TierPrices.FirstOrDefault(p => p.CustomerRoleId == (int)ProductPriceTier.B)?.Price ?? 0;
                        productModel.TierPriceC = product.TierPrices.FirstOrDefault(p => p.CustomerRoleId == (int)ProductPriceTier.C)?.Price ?? 0;
                        //productModel.TierPricesText = "A: " + product.TierPrices.FirstOrDefault(p => p.CustomerRoleId == (int)ProductPriceTier.A)?.Price.ToString("#.00") + "<br />" +
                        //                              "B: " + product.TierPrices.FirstOrDefault(p => p.CustomerRoleId == (int)ProductPriceTier.B)?.Price.ToString("#.00") + "<br />" +
                        //                              "C: " + product.TierPrices.FirstOrDefault(p => p.CustomerRoleId == (int)ProductPriceTier.C)?.Price.ToString("#.00") + "<br />";
                    }
        ...
        public virtual ProductOrderListModel PrepareProductOrderListModel(ProductOrderSearchModel searchModel, Product product)
        {
            ...
            //get orders
            var orders = _orderService.SearchOrders2(productId: searchModel.ProductId, salemanId: searchModel.SalesmanId,
                pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);
            ...
            var model = new ProductOrderListModel().PrepareToGrid(searchModel, orders, () =>
            {
                return orders.Select(order =>
                {
                    ...
                    orderModel.ItemQuantity = order.OrderItems.FirstOrDefault(i => i.ProductId == product.Id)?.Quantity ?? 0;
