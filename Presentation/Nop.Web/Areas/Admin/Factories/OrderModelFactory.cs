...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
        ...
        protected virtual void PrepareOrderItemModels(IList<OrderItemModel> models, Order order)
        {
            ...
            foreach (var orderItem in orderItems)
            {
                ...
                var orderItemModel = new OrderItemModel
                {
                    ...
                    CategoriesText = string.Join(" | ", orderItem.Product.ProductCategories.Select(p => p.Category.Name).ToList()),
                    SelectedCategoryIds = orderItem.Product.ProductCategories.Select(p => p.Category.Id).ToList(),
                    ...
                    //  SubTotalExclTaxValue = orderItem.PriceExcTax,
        ...
        protected virtual void PrepareOrderModelTotals(OrderModel model, Order order)
        {
            ...
            if (!order.Profit.HasValue || order.Profit == 0)
            {
                grossProfit = _orderReportService.ProfitReport(orderId: order.Id);
                netProfit = _orderReportService.NetProfitReport(orderId: order.Id);
            }
            model.GrossProfit = _priceFormatter.FormatPrice(grossProfit.Value, true, false);
            model.NetProfit = _priceFormatter.FormatPrice(netProfit.Value, true, false);
            model.ActualShippingCost = order.ActualShippingCost != null ? order.ActualShippingCost.Value : 0;
            model.ActualShippingCostStr = _priceFormatter.FormatPrice(model.ActualShippingCost, true, false);
        ...
        protected virtual void PrepareOrderModelPaymentInfo(OrderModel model, Order order)
        {
            ...
            { model.PaymentDueOn = _dateTimeHelper.ConvertToUserTime(order.CreatedOnUtc.AddDays(7), DateTimeKind.Utc).ToShortDateString(); }
            else if (order.PaymentTermId == 2)
            { model.PaymentDueOn = _dateTimeHelper.ConvertToUserTime(order.CreatedOnUtc.AddDays(14), DateTimeKind.Utc).ToShortDateString(); }
            else if (order.PaymentTermId == 3)
            { model.PaymentDueOn = _localizationService.GetResource("Admin.Orders.PaymentTerm.COD"); }
            else if (order.PaymentTermId == 4)
            { model.PaymentDueOn = _localizationService.GetResource("Admin.Orders.PaymentTerm.OSA"); }
        }
        ...
        public virtual OrderSearchModel PrepareOrderSearchModel(OrderSearchModel searchModel)
        {
            ...
            // prepare Customer Roles List
            searchModel.AvailableCustomerRoles = _customerService.GetAllCustomerRoles().Select(customer => new SelectListItem
            {
            ...
            searchModel.AvailableCustomerRoles.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
        ...
        public virtual OrderListModel PrepareOrderListModel(OrderSearchModel searchModel)
        {
            ...
            //get orders
            var orders = _orderService.SearchOrders2(storeId: searchModel.StoreId,
                vendorId: searchModel.VendorId,
                salemanId: searchModel.SalemanId,
            ...
            var model = new OrderListModel().PrepareToGrid(searchModel, orders, () =>
            {
                ...
                return orders.Select(order =>
                {
                    ...
                    var profit = _orderReportService.NetProfitReport(orderId: order.Id);
                    orderModel.NetProfit = _priceFormatter.FormatPrice(profit, true, false);
                    return orderModel;
                });
            });
        ...
        public virtual OrderAggreratorModel PrepareOrderAggregatorModel(OrderSearchModel searchModel)
        {
            ...
            var profitSum = _priceFormatter.FormatPrice(reportSummary.SumNetProfit, true, false);
        ...
        public virtual OrderModel PrepareOrderModel(OrderModel model, Order order, bool excludeProperties = false)
        {
            if (order != null)
            {
                ...
                model.NoteForAccounts = order.NoteForAccounts;
                model.NoteForWarehouse = order.NoteForWarehouse;

                model.OrderStatus = _localizationService.GetLocalizedEnum(order.OrderStatus);
                //model.CustomerRoleNames = _localizationService.GetLocalizedEnum(order.OrderStatus);
        ...
        /// <summary>
        /// Prepare  Quick order model
        /// </summary>
        /// <param name="model">Order model</param>
        /// <param name="order">Order</param>
        /// <param name="excludeProperties">Whether to exclude populating of some properties of model</param>
        /// <returns>Order model</returns>
        public virtual OrderModel PrepareQuickOrderModel(OrderModel model, Order order, bool excludeProperties = false)
        {
            if (order != null)
            {
                //fill in model values from the entity
                ...
                // model.OrderStatus = _localizationService.GetLocalizedEnum();
                _baseAdminModelFactory.PrepareOrderStatuses(model.AvailableOrderStatuses, defaultItemText: null);
                _baseAdminModelFactory.PreparePaymentTerms(model.AvailablePaymentTerms, defaultItemText: null);
                _baseAdminModelFactory.PrepareCategories(model.AvailableCategories);

                model.AvailableOrderStatuses = model.AvailableOrderStatuses.Where(x => Convert.ToString(x.Value) != ((int)OrderStatus.Cancelled).ToString()).ToList();
                //  model.CustomerRoleNames = _localizationService.GetLocalizedEnum(order.OrderStatus);
                // model.PaymentStatus = _localizationService.GetLocalizedEnum(order.PaymentStatus);
                model.CustomersList = model.CustomersList ?? new List<CustomerSalemanListItem>();
                model.SalemanList = model.SalemanList ?? new List<CustomerSalemanListItem>();

                //// Price Group
                //model.PriceGroupsList = _customerService.GetAllCustomerRoles().Where(x=>x.SystemName.ToLower().Contains(NopCustomerDefaults.CustomerRoleName.ToLower())).Select(customer => new SelectListItem
                //{
                //    Value = customer.Id.ToString(),
                //    Text = customer.Name
                //}).ToList();

                //  model.PriceGroupsList.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.None"), Value = "0" });
                // Customer 

                _baseAdminModelFactory.PrepareCustomersWRTRoleQuickOrder(model.CustomersList, needCustomersWithRole: NopCustomerDefaults.CustomerRoleName,orderCustomerId:order.CustomerId, CurrentUserId: _workContext.CurrentCustomer.Id,
               defaultItemText: null);;

                // Saleman
                _baseAdminModelFactory.PrepareCustomersWRTRoleQuickOrder(model.SalemanList, needCustomersWithRole: NopCustomerDefaults.SalemanRoleName, CurrentUserId: _workContext.CurrentCustomer.Id,
                defaultItemText: " ");
                // _baseAdminModelFactory.PrepareCustomersWRTRole(model.SalemanList, role: NopCustomerDefaults.SalemanRoleName,
                //defaultItemText: " ");
            ...
            var paymentMethods = _paymentPluginManager
                .LoadActivePlugins(_workContext.CurrentCustomer)
                .Where(pm => pm.PaymentMethodType == PaymentMethodType.Standard || pm.PaymentMethodType == PaymentMethodType.Redirection)
                .ToList();
            model.PaymentMethodList = new List<SelectListItem>();
            model.PaymentMethod = order.PaymentMethodSystemName;
            foreach (var pm in paymentMethods)
            {
                var name = _localizationService.GetLocalizedFriendlyName(pm, _workContext.WorkingLanguage.Id);
                var systemName = pm.PluginDescriptor.SystemName;
                model.PaymentMethodList.Add(new SelectListItem { Text = name, Value = systemName });
            }
            return model;
        }
        /// <summary>
        ...
        /// <param name="excludeProperties">Whether to exclude populating of some properties of model</param>
        /// <returns>Order model</returns>
        public virtual OrderModel PrepareNewQuickOrderModel(OrderModel model, bool excludeProperties = false)
        {
            Customer loggedInCustomer = _workContext.CurrentCustomer;
            //fill in model values from the entity
            model = model ?? new OrderModel
            {
                //Id = order.Id,
                OrderStatusId = 1,
                VatNumber = _workContext.CurrentCustomer.LastIpAddress,
                CheckoutAttributeInfo = String.Empty
            };

            model.OrderGuid = Guid.NewGuid();
            model.CustomOrderNumber = _workContext.CurrentCustomer.LastIpAddress;
            model.CustomerIp = _workContext.CurrentCustomer.LastIpAddress;
            if (model.CustomerId <= 0)
                model.CustomerId = model.CustomerId == 0 ? _workContext.CurrentCustomer.Id : model.CustomerId;
            ...
            //  model.CustomerRoleNames = _localizationService.GetLocalizedEnum(order.OrderStatus);
            // model.PaymentStatus = _localizationService.GetLocalizedEnum(order.PaymentStatus);
            model.CustomersList = model.CustomersList == null ? new List<CustomerSalemanListItem>() : model.CustomersList;
            model.SalemanList = model.SalemanList == null ? new List<CustomerSalemanListItem>() : model.SalemanList;
            //  model.PriceGroupsList.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.None"), Value = "0" });
            // Customer 

            _baseAdminModelFactory.PrepareCustomersWRTRoleQuickOrder(model.CustomersList, needCustomersWithRole: NopCustomerDefaults.CustomerRoleName, CurrentUserId: _workContext.CurrentCustomer.Id,
           defaultItemText: null);
            ...
            model.StoreName = _storeService.GetStoreById(_workContext.CurrentCustomer.RegisteredInStoreId)?.Name ?? "Deleted";
            model.CustomerInfo = _workContext.CurrentCustomer.IsRegistered() ? _workContext.CurrentCustomer.Email : _localizationService.GetResource("Admin.Customers.Guest");
            model.CreatedOn = _dateTimeHelper.ConvertToUserTime(System.DateTime.Now, DateTimeKind.Utc);
            // model.CustomValues = _paymentService.DeserializeCustomValues(order);
            ...
            model.AllowCustomersToSelectTaxDisplayType = _taxSettings.AllowCustomersToSelectTaxDisplayType;
            model.TaxDisplayType = _taxSettings.TaxDisplayType;
            model.CustomersList = model.CustomersList.OrderBy(a => a.Text).ToArray();
            model.SalemanList = model.SalemanList.OrderBy(a => a.Text).ToArray();
            //filter by country
            var paymentMethods = _paymentPluginManager
                .LoadActivePlugins(_workContext.CurrentCustomer)
                .Where(pm => pm.PaymentMethodType == PaymentMethodType.Standard || pm.PaymentMethodType == PaymentMethodType.Redirection)
            ...
            foreach (var pm in paymentMethods)
            {
                var name = _localizationService.GetLocalizedFriendlyName(pm, _workContext.WorkingLanguage.Id);
                var systemName = pm.PluginDescriptor.SystemName;
                model.PaymentMethodList.Add(new SelectListItem { Text = name, Value = systemName, Selected = (systemName == "Payments.CheckMoneyOrder") });
            }
            return model;
        }
        /// <summary>
        ...
        public virtual BestsellerBriefListModel PrepareBestsellerBriefListModel(BestsellerBriefSearchModel searchModel)
        {
            ...
            //get bestsellers
            var bestsellers = _orderReportService.BestSellersReport2(showHidden: true,
                vendorId: _workContext.CurrentVendor?.Id ?? 0,
                salesmanId: searchModel.SalesmanId,
        ...
        public virtual OrderAverageReportListModel PrepareOrderAverageReportListModel(OrderAverageReportSearchModel searchModel)
        {
            ...
            {
                _orderReportService.OrderAverageReport2(0, OrderStatus.Pending, searchModel.SalesmanId),
                _orderReportService.OrderAverageReport2(0, OrderStatus.Processing, searchModel.SalesmanId),
                _orderReportService.OrderAverageReport2(0, OrderStatus.Complete, searchModel.SalesmanId),
                _orderReportService.OrderAverageReport2(0, OrderStatus.Cancelled, searchModel.SalesmanId)
            };
        ...
        public virtual OrderIncompleteReportListModel PrepareOrderIncompleteReportListModel(OrderIncompleteReportSearchModel searchModel)
        {
            ...
            var psPending = _orderReportService.GetOrderAverageReportLine2(psIds: paymentStatuses, osIds: orderStatuses, salesmanId: searchModel.SalesmanId);
            ...
            var ssPending = _orderReportService.GetOrderAverageReportLine2(osIds: orderStatuses, ssIds: shippingStatuses, salesmanId: searchModel.SalesmanId);
            orderIncompleteReportModels.Add(new OrderIncompleteReportModel
            {
            ...
            var osPending = _orderReportService.GetOrderAverageReportLine2(osIds: orderStatuses, salesmanId: searchModel.SalesmanId);
            orderIncompleteReportModels.Add(new OrderIncompleteReportModel
            {
