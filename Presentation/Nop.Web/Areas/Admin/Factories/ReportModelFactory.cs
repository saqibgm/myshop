...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class ReportModelFactory : IReportModelFactory
    {
        ...
        /// <param name="searchModel">Country report search model</param>
        /// <returns>Country report search model</returns>
        public virtual SalemanCommissionReportSearchModel PrepareSalemanCommissionSearchModel(SalemanCommissionReportSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            searchModel.SalemanList = searchModel.SalemanList == null ? new List<CustomerSalemanListItem>() : searchModel.SalemanList;
            _baseAdminModelFactory.PrepareCustomersWRTRoleQuickOrder(searchModel.SalemanList, needCustomersWithRole: NopCustomerDefaults.SalemanRoleName, CurrentUserId: _workContext.CurrentCustomer.Id,
           defaultItemText: " ");
            //searchModel.SalemanList.OrderBy(a => a.Text).ToArray();
            searchModel.SalemanList.Insert(0, new CustomerSalemanListItem { Text = _localizationService.GetResource("Admin.Common.All"), Value = "0" });
            //prepare available order statuses
            _baseAdminModelFactory.PrepareOrderStatuses(searchModel.AvailableOrderStatuses);
        ...
        /// <param name="searchModel">Country report search model</param>
        /// <returns>Country report list model</returns>
        public virtual SalemanCommissionReportListModel PrepareSalemanCommissionListModel(SalemanCommissionReportSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));

            //get parameters to filter countryReports
            searchModel.SearchKeyword = "" + searchModel.SearchKeyword;
            var orderStatus = searchModel.OrderStatusId > 0 ? (OrderStatus?)searchModel.OrderStatusId : null;
            var paymentStatus = searchModel.PaymentStatusId > 0 ? (PaymentStatus?)searchModel.PaymentStatusId : null;
            var startDateValue = searchModel.StartDate; //!searchModel.StartDate.HasValue ? null : (DateTime?)_dateTimeHelper.ConvertToUtcTime(searchModel.StartDate.Value, _dateTimeHelper.CurrentTimeZone);
            var endDateValue = searchModel.EndDate.Value.AddDays(1).AddMilliseconds(-1);//!searchModel.EndDate.HasValue ? null : (DateTime?)_dateTimeHelper.ConvertToUtcTime(searchModel.EndDate.Value, _dateTimeHelper.CurrentTimeZone);

            //get items
            var items = _orderReportService.GetCommissionReport( os: orderStatus, salemanId: searchModel.SalemanId,
                ps: paymentStatus,
                startTimeUtc: startDateValue,
                endTimeUtc: endDateValue).ToPagedList(searchModel);

            //prepare list model
            var commissionModel = new SalemanCommissionReportListModel().PrepareToGrid(searchModel, items, () =>
            {
                return items.Select(item =>
                {
                    //fill in model values from the entity
                    var commissionReportModel = new SalemanCommissionReportModel                    {
                       
                    };

                    //fill in additional values (not existing in the entity)
                    commissionReportModel.SalemanName = _customerService.GetCustomerFullName(_customerService.GetCustomerById(Convert.ToInt32(item.SalemanId)));  
                    commissionReportModel.TotalSaleAmount = _priceFormatter.FormatPrice(item.TotalSale, true, false);
                    commissionReportModel.TotalOrders = item.TotalOrders;
                    commissionReportModel.CommissionAmount = _priceFormatter.FormatPrice(item.CommissionAmount, true, false);
                    commissionReportModel.TotalSalemanSalaryIncCommission = _priceFormatter.FormatPrice(item.TotalSalemanSalaryIncCommission, true, false);
                    commissionReportModel.TotalProfit = _priceFormatter.FormatPrice(item.TotalProfit, true, false);
                    

                    return commissionReportModel;
                }).Where(x=>x.SalemanName.ToLower().Contains(searchModel.SearchKeyword.Trim()));
            });
        ...
        /// Prepare country report search model
        /// </summary>
        /// <param name="searchModel">Profit report search model</param>
        /// <returns>Profit report search model</returns>
        public virtual ProfitReportSearchModel PrepareProfitSearchModel(ProfitReportSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            _baseAdminModelFactory.PrepareCustomers(searchModel.AvailableCustomers, role: NopCustomerDefaults.CustomerRoleName,
        ...
        /// <summary>
        /// Prepare paged Profit Report
        /// </summary>
        /// <param name="searchModel">Profit Report report search model</param>
        /// <returns>Customer Profile Report ILIST</returns>
        public virtual ProfitReportListModel PrepareProfitListModel(ProfitReportSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));

            //get parameters to filter countryReports
          var orderStatus = Convert.ToInt32(searchModel.OrderStatusId) >0? searchModel.OrderStatusId : 0;
            ...
            //get items int storeId, int CustomerId, string InvoiceNo, DateTime startTimeUtc, DateTime endTimeUtc
            var items = _orderReportService.GetCustomerProfitReport(storeId:0, CustomerId: searchModel.CustomerId, OrderStatusId: orderStatus, InvoiceNo:string.Empty, 
                startTimeUtc: startDateValue,
                endTimeUtc: endDateValue).ToPagedList(searchModel);

            //prepare list model
            var model = new ProfitReportListModel().PrepareToGrid(searchModel, items, () =>
            {
                return items.Select(item =>
                {
                    //fill in model values from the entity
                    var ProfitReportModel = new ProfitReportModel
                    {
                        CustomerName = item.CustomerName,
                        TotalAmount = item.ProfitTotal,                   
                        InvoiceCount = item.InvoiceCount
                    };
                    //fill in additional values (not existing in the entity)
                    ProfitReportModel.TotalAmountStr = _priceFormatter.FormatPrice(item.ProfitTotal, true, false);
                    ProfitReportModel.CustomerName = _customerService.GetCustomerFullName(_customerService.GetCustomerById(item.CustomerId));
                    ProfitReportModel.TotalSaleStr = _priceFormatter.FormatPrice(item.TotalSale, true, false);
                    ProfitReportModel.ProfitStr = _priceFormatter.FormatPrice(item.Profit, true, false);
                    ProfitReportModel.TotalAmountStr = _priceFormatter.FormatPrice(item.ProfitTotal, true, false);//item.TotalOrders//_priceFormatter.FormatPrice(item.SumOrders, true, false);
                                                                                                        //  ProfitReportModel.TotalSale = item. _countryService.GetCountryById(item.SumOrders ?? 0)?.Name;
        ...
        /// Prepare paged best customers report list model
        /// </summary>
        /// <param name="searchModel">Saleman Profile report search model</param>
        /// <returns>Saleman Profile report list model</returns>
        public virtual CustomerProfileListModel PrepareSalemanStatisticsList(CustomerProfileSearchModel SearchModel)
        {
            if (SearchModel == null)
                throw new ArgumentNullException(nameof(SearchModel));
          /************************ DATEs**********************************/
          // One Year Date Criteria
            DateTime startDateValue = new DateTime(DateTime.Now.Year - 1, DateTime.Now.Month, 1);
            DateTime endDateValue = DateTime.Now;


        var reportItems = _orderReportService.GetSalemanStatisticsReport(storeId: 0,  SalemanId: SearchModel.SalemanId, startTimeUtc: startDateValue, endTimeUtc: endDateValue);

            ///******************************** PREPARE GRID DATA****************************/
            //List<CustomerProfileBindingModel> model = new List<CustomerProfileBindingModel>();
            //foreach (OrderBySalemanStatisticsReportLine item in reportItems)
            //{

            //    CustomerProfileBindingModel profileStats = new CustomerProfileBindingModel();
            //    profileStats.CommissionStr = _priceFormatter.FormatPrice(item.Commission, true, false);
            //    profileStats.MonthName = item.MonthName;
            //    profileStats.TotalSaleStr = _priceFormatter.FormatPrice(item.TotalSale, true, false);
            //    profileStats.UniqueCustomer = item.UniqueCustomer;
            //    profileStats.Order = item.Order;
            //    model.Add(profileStats);
            //}
            var pagedList = reportItems.ToPagedList(SearchModel);

            var model = new CustomerProfileListModel().PrepareToGrid(SearchModel, pagedList, () =>
            {
                return reportItems.Select(item =>
                {
                    //fill in model values from the entity
                    var CustomerProfile = new CustomerProfileModel
                    {
                        ...
                        TotalSaleStr = _priceFormatter.FormatPrice(item.TotalSale, true, false),
                        Commission = item.Commission,
                        CommissionStr=_priceFormatter.FormatPrice(item.Commission, true, false),
                        ProfitStr = _priceFormatter.FormatPrice(item.TotalProfit, true, false),
