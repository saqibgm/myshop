...
namespace Nop.Web.Areas.Admin.Factories
{
    ...
    public partial class CustomerModelFactory : ICustomerModelFactory
    {
        ...
        protected virtual void PrepareCustomerAttributeModels(IList<CustomerModel.CustomerAttributeModel> models, Customer customer)
        {
            ...
            var customerAttributes = _customerAttributeService.GetAllCustomerAttributes().OrderBy(a => a.DisplayOrder);
        ...
        /// Prepare customer order search model
        /// </summary>
        /// <param name="searchModel">Customer Commission search model</param>
        /// <param name="customer">Customer</param>
        /// <returns>Customer order search model</returns>
        protected virtual CustomerCommissionSearchModel PrepareCustomerCommissionSearchModel(CustomerCommissionSearchModel searchModel, Customer customer)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            searchModel.SalemanId = customer.Id;
        ...
        public virtual CustomerProfileSearchModel PrepareCustomerProfileModel(CustomerProfileSearchModel searchModel)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));

            Customer model = _customerService.GetCustomerById(searchModel.SalemanId);
            /************************  ALL DATA ABOUT SALE MAN ******************************/
            searchModel.SurName = _customerService.GetCustomerFullName(model);
            searchModel.SID = String.Format("{0:D6}", model.Id);
            searchModel.BasicSalary = _priceFormatter.FormatPrice((Convert.ToDecimal("" + model.BasicSalary == string.Empty ? "0.0" : "" + model.BasicSalary)), true, false);
            //searchModel.Avator = model.pi;
            searchModel.CommissionPercentage = Convert.ToString(Math.Round(model.CommissionPercentage ?? 0, 1));
            searchModel.CommissionMinTargert = _priceFormatter.FormatPrice(model.CommissionMinTargert ?? 0, true, false);
            searchModel.Email = model.Email;
            searchModel.Phone = "";
            Picture picCustomer = _pictureService.GetPictureById(model.AffiliateId);
            Address cAddressSaleman = model.Addresses.Where(x => x.PhoneNumber != null && x.PhoneNumber != string.Empty).FirstOrDefault();
            if (cAddressSaleman != null)
            {
                searchModel.Phone = cAddressSaleman.PhoneNumber;
            }

            if (_customerSettings.AllowCustomersToUploadAvatars)
            {
                var avatarPictureId = _genericAttributeService.GetAttribute<int>(model, NopCustomerDefaults.AvatarPictureIdAttribute);
                searchModel.Avator = _pictureService.GetPictureUrl(avatarPictureId, _mediaSettings.AvatarPictureSize,
                    _customerSettings.DefaultAvatarEnabled, defaultPictureType: PictureType.Avatar);
            }
            /******************************* MANAGER *********************************************/
            if (model.ReportingTo > 0)
            {
                Customer Manager = _customerService.GetCustomerById(model.ReportingTo);
                /************************  ALL DATA ABOUT SALE MAN ******************************/
                searchModel.SurNameMngr = _customerService.GetCustomerFullName(Manager);
                searchModel.SIDMngr = String.Format("{0:D6}", Manager.Id);
                searchModel.EmailMngr = Manager.Email;
                searchModel.PhoneMngr = "";

                Address cAddressMngr = Manager.Addresses.Where(x => x.PhoneNumber != null && x.PhoneNumber != string.Empty).FirstOrDefault();
                if (cAddressMngr != null)
                {
                    searchModel.PhoneMngr = cAddressMngr.PhoneNumber;
                }
                // Get Picture of Manager
                if (_customerSettings.AllowCustomersToUploadAvatars)
                {
                    var avatarPictureId = _genericAttributeService.GetAttribute<int>(Manager, NopCustomerDefaults.AvatarPictureIdAttribute);
                    searchModel.AvatorMngr = _pictureService.GetPictureUrl(avatarPictureId, _mediaSettings.AvatarPictureSize,
                        _customerSettings.DefaultAvatarEnabled, defaultPictureType: PictureType.Avatar);
                }
            }// Manager Existance
            return searchModel;
        ...
        public virtual CustomerSearchModel PrepareCustomerSearchModel(CustomerSearchModel searchModel)
        {
            ...
            searchModel.AvailableSalemanList = new List<SelectListItem>();
            // Prepare Saleman List
            _baseAdminModelFactory.PrepareCustomersWRTRole(searchModel.AvailableSalemanList, role: NopCustomerDefaults.SalemanRoleName,
                  defaultItemText: _localizationService.GetResource("Admin.Common.All"));
            //insert this default item at first
            //searchModel.AvailableSalemanList.Insert(0, new SelectListItem { Text = _localizationService.GetResource("Admin.Common.All"), Value = "" });
            //prepare page parameters
            searchModel.SetGridPageSize();
        ...
        public virtual CustomerListModel PrepareCustomerListModel(CustomerSearchModel searchModel)
        {
            ...
            //get customers
            var customers = _customerService.GetAllCustomers2(customerRoleIds: searchModel.SelectedCustomerRoleIds.ToArray(),
                ...
                country: searchModel.SearchCountry,
                salesmanid: searchModel.SearchSalesman,
                pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize, itemsInCart: searchModel.ItemsInCart, customerStatus: searchModel.SearchStatus);
            ...
            var model = new CustomerListModel().PrepareToGrid(searchModel, customers, () =>
            {
                return customers.Select(customer =>
                {
                    ...
                    customerModel.City = _genericAttributeService.GetAttribute<string>(customer, NopCustomerDefaults.CityAttribute);
                    ...
                    if (_customerSettings.AllowCustomersToUploadAvatars)
                    {
                        var pictureSize = searchModel.AvatarSize != 0 ? searchModel.AvatarSize : _mediaSettings.AvatarPictureSize;
                        var avatarPictureId = _genericAttributeService.GetAttribute<int>(customer, NopCustomerDefaults.AvatarPictureIdAttribute);
                        customerModel.AvatarUrl = _pictureService.GetPictureUrl(avatarPictureId, pictureSize,
                            _customerSettings.DefaultAvatarEnabled, defaultPictureType: PictureType.Avatar);
                    }
        ...
        public virtual CustomerModel PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties = false)
        {
            if (customer != null)
            {
                ...
                model.IsRegistered = customer.IsRegistered();
                model.IsCustomer = customer.IsCustomer();
                model.IsAdmin = customer.IsAdmin();
                model.IsManager = customer.IsManager();
                model.IsSalesman = customer.IsSaleman();
                ...
                if (!excludeProperties)
                {
                    ...
                    model.ReportingTo = customer.ReportingTo;
                    model.TeamMemberOfSaleman = customer.TeamMemberOfSaleman;
                    model.AdminComment = customer.AdminComment;
                    model.IsTaxExempt = customer.IsTaxExempt;
                    model.BasicSalary = customer.BasicSalary;
                    model.CommissionPercentage = customer.CommissionPercentage;
                    model.CommissionMinTargert = customer.CommissionMinTargert;
                    model.IsExportAllowed = customer.IsExportAllowed;
                    ...
                    //model.BusinessCertUrl = _pictureService.GetPictureUrl(_genericAttributeService.GetAttribute<int>(customer, NopCustomerDefaults.BusinessCertIdAttribute), 0, false);
                    //model.BusinessCertUrl = Url.Action("GetFileUploadById", "Download", new { downloadId = model.BusinessCertId });
                    ...
                    model.BusinessCertId = customer.BusinessCertId;
                ...
                PrepareCustomerCommissionSearchModel(model.CustomerCommissionSearchModel, customer);
            ...
            _baseAdminModelFactory.PrepareCustomersWRTRole(model.AvailableManagers, role: NopCustomerDefaults.ManagerRoleName,
                 defaultItemText: _localizationService.GetResource("Admin.Customers.Customers.Fields.Manager.None"));
            // Prepare Saleman List
            _baseAdminModelFactory.PrepareCustomersWRTRole(model.AvailableSalemanList, role: NopCustomerDefaults.SalemanRoleName,
                  defaultItemText: _localizationService.GetResource("admin.customers.customer.fields.common.none"));
        ...
        /// <param name="customer">Customer</param>
        /// <returns>Customer order list model</returns>
        public virtual CustomerCommissionListModel PrepareSalemanCommissionListModel(CustomerCommissionSearchModel searchModel, Customer customer)
        {
            if (searchModel == null)
                throw new ArgumentNullException(nameof(searchModel));
            ...
            //get customer orders
            var commissions = _customerService.GetSalemanCommissions(salemanId: searchModel.SalemanId,
                pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);

            //prepare list model
            var model = new CustomerCommissionListModel().PrepareToGrid(searchModel, commissions, () =>
            {
                return commissions.Select(commission =>
                {
                    //fill in model values from the entity
                    var commissionModel = commission.ToModel<CustomerCommissionModel>();

                    //convert dates to the user time
                    commissionModel.CreatedOnUtc = _dateTimeHelper.ConvertToUserTime(commission.CreatedOnUtc, DateTimeKind.Utc);

                    //fill in additional values (not existing in the entity)
                    commissionModel.CommissionMinTargert = commission.CommissionMinTargert ?? 0;
                    commissionModel.CommissionMonth = commission.CommissionMonth;
                    commissionModel.CommissionPercentage = commission.CommissionPercentage ?? 0;
                    commissionModel.CustomerId = commission.CustomerId;
                    commissionModel.Id = commission.Id;
                    commissionModel.TotalCommission = commission.TotalCommission;
                    commissionModel.TotalOrders = commission.TotalOrders;
                    commissionModel.TotalProfit = commission.TotalProfit;
                    commissionModel.TotalSalary = commission.TotalSalary;
                    commissionModel.TotalSale = commission.TotalSale;
                    commissionModel.BasicSalary = commission.BasicSalary ?? 0;
                    commissionModel.CommissionMinTargertStr = _priceFormatter.FormatPrice(commission.CommissionMinTargert ?? 0, true, false);
                    commissionModel.TotalCommissionStr = _priceFormatter.FormatPrice(commission.TotalCommission, true, false);
                    commissionModel.TotalProfitStr = _priceFormatter.FormatPrice(commission.TotalProfit, true, false);
                    commissionModel.TotalSalaryStr = _priceFormatter.FormatPrice(commission.TotalSalary, true, false);
                    commissionModel.TotalSaleStr = _priceFormatter.FormatPrice(commission.TotalSale, true, false);
                    commissionModel.BasicSalaryStr = _priceFormatter.FormatPrice(commission.BasicSalary??0, true, false);
                    commissionModel.CommissionMonthStr = commission.CommissionMonth.ToString("yyyy MMMM");
            ...
            Customer customer)
        {
            ...
            var pageList = shoppingCart.Select(item =>
            {
                ...
                //fill in additional values (not existing in the entity)
                shoppingCartItemModel.CategoriesText = string.Join(" | ", item.Product.ProductCategories.Select(p => p.Category.Name).ToList());
                shoppingCartItemModel.SelectedCategoryIds = item.Product.ProductCategories.Select(p => p.Category.Id).ToList();
                shoppingCartItemModel.Sku = item.Product.Sku;
                shoppingCartItemModel.ProductName = item.Product.Name;
                shoppingCartItemModel.StockQuantity = item.Product.StockQuantity;
                ...
                shoppingCartItemModel.UnitPriceDec = _priceCalculationService.GetUnitPrice(item);
                shoppingCartItemModel.UnitPrice = _priceFormatter.FormatPrice(_taxService.GetProductPrice(item.Product, shoppingCartItemModel.UnitPriceDec, out var _));
            ...
            CustomerBackInStockSubscriptionSearchModel searchModel, Customer customer)
        {
            ...
            var pageList = subscriptions.Select(subscription =>
            {
                ...
                subscriptionModel.StockQuantity = subscription.Product?.StockQuantity ?? 0;
        ...
        public virtual OnlineCustomerListModel PrepareOnlineCustomerListModel(OnlineCustomerSearchModel searchModel)
        {
            ...
            var customers = _customerService.GetOnlineCustomers(customerRoleIds: new int[] { 3 },
                 lastActivityFromUtc: lastActivityFrom,
                 pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);
            //prepare list model
            if (_workContext.CurrentCustomer.IsSaleman())
            {
                customers = _customerService.GetOnlineCustomersForSalesMan(customerRoleIds: new int[] { 3 }, salesmanId: _workContext.CurrentCustomer.Id,
                 lastActivityFromUtc: lastActivityFrom,
                 pageIndex: searchModel.Page - 1, pageSize: searchModel.PageSize);
            }
