        ...
        public virtual ShoppingCartListModel PrepareShoppingCartListModel(ShoppingCartSearchModel searchModel)
        {
            ...
            var model = new ShoppingCartListModel().PrepareToGrid(searchModel, customers, () =>
            {
                return customers.Select(customer =>
                {
                    ...
                    var cartItems = _shoppingCartService.GetShoppingCart(customer, searchModel.ShoppingCartType, searchModel.StoreId, searchModel.ProductId, searchModel.StartDate, searchModel.EndDate);
                    shoppingCartModel.TotalItems = cartItems.Sum(item => item.Quantity);
                    var totalValue = cartItems.Sum(item => item.Quantity * _productService.GetPreferredTierPrice(item.Product, customer, searchModel.StoreId, item.Quantity)?.Price ?? item.Product.Price);
                    shoppingCartModel.TotalValue = _priceFormatter.FormatPrice(totalValue, true, _workContext.WorkingCurrency.CurrencyCode, false, _workContext.WorkingLanguage);
                    var availableValue = cartItems.Sum(item => ((item.Product?.StockQuantity ?? 0) > 0 ? item.Quantity * _productService.GetPreferredTierPrice(item.Product, customer, searchModel.StoreId, item.Quantity)?.Price ?? item.Product.Price : 0) );
                    shoppingCartModel.AvailableValue = _priceFormatter.FormatPrice(availableValue, true, _workContext.WorkingCurrency.CurrencyCode, false, _workContext.WorkingLanguage);
        ...
        public virtual ShoppingCartItemListModel PrepareShoppingCartItemListModel(ShoppingCartItemSearchModel searchModel, Customer customer)
        {
            ...
            var model = new ShoppingCartItemListModel().PrepareToGrid(searchModel, items, () =>
            {
                return items.Select(item =>
                {
                    ...
                    itemModel.StockQuantity = item.Product.StockQuantity;
                    itemModel.CategoriesText = string.Join(" | ", item.Product.ProductCategories.Select(p => p.Category.Name).ToList());
                    itemModel.SelectedCategoryIds = item.Product.ProductCategories.Select(p => p.Category.Id).ToList();
                    itemModel.Sku = item.Product.Sku;
                    //fill in additional values (not existing in the entity)
